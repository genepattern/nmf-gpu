...
try:
  for k in range(mink,maxk + 1):
    ...
    for seed in seed_list:
      ...
      #cmd = f'jsrun --smpiargs="-gpu" --nrs={mpitasks} --tasks_per_rs=1 --cpu_per_rs=1 --gpu_per_rs=1 --rs_per_host={mpitasks} --bind=rs {GPUPROGRAMPATH}  --inputmatrix=test/bionmf.input.txt  -k {k}  -j 10  -t 40  -i 2000 -s {seed} > /gpfs/wolf/trn008/scratch/kenneth/nmf-gpu/kenneth/bin/mpi.out 2>/gpfs/wolf/trn008/scratch/kenneth/nmf-gpu/kenneth/bin/mpi.err'
      if debug:
        DEBUGOPTION = '--verbose'
      else:
        DEBUGOPTION = ''
      #cmd = f'{GPUPROGRAMPATH} {DEBUGOPTION} --inputmatrix=test/bionmf.input.txt  -k {k}  -j 10  -t 40  -i 2000 -s {seed} > /gpfs/wolf/trn008/scratch/kenneth/nmf-gpu/kenneth/bin/mpi.out 2>/gpfs/wolf/trn008/scratch/kenneth/nmf-gpu/kenneth/bin/mpi.err'
      ...
      #cmd = f'{GPUPROGRAMPATH} {DEBUGOPTION} --inputmatrix=test/bionmf.input.txt  -k {k}  -j 10  -t 40  -i 2000 -s {seed} > {JOBDIR}/mpi.out 2>{JOBDIR}/mpi.err'
      #cmd = f'jsrun -E LD_PRELOAD=/opt/ibm/spectrum_mpi/lib/pami_490/libpami.so --smpiargs="-gpu" --nrs={mpitasks} --tasks_per_rs=1 --cpu_per_rs=1 --gpu_per_rs=1 --rs_per_host={mpitasks} --bind=rs {GPUPROGRAMPATH} {DEBUGOPTION} --inputmatrix=test/bionmf.input.txt  -k {k}  -j 10  -t 40  -i 2000 -s {seed} > {JOBDIR}/mpi.out 2>{JOBDIR}/mpi.err'
      cmd = f'jsrun --smpiargs="-gpu" --nrs={mpitasks} --tasks_per_rs=1 --cpu_per_rs=1 --gpu_per_rs=1 --rs_per_host={mpitasks} --bind=rs {GPUPROGRAMPATH} {DEBUGOPTION} --inputmatrix=test/bionmf.input.txt  -k {k}  -j 10  -t 40  -i 2000 -s {seed} > {JOBDIR}/mpi.out 2>{JOBDIR}/mpi.err'
      if debug:
        print(f'cmd: ({cmd})\n')
      os.system(cmd)
      ...
      #  sys.stderr.write(f'input_line: ({input_line})\n')
      ...
      #    #if maxrow_list[field_index][1] <= field_float:
      #    #  maxrow_list[field_index][1] = field_float
      #    #  maxrow_list[field_index][0] = input_line_index
